...
document.querySelector('#show-item .bg-blue-500').addEventListener('click', function () {
    ...
    if (existingRow) {
        ...
        const selectedSizeName = document.querySelector(`#size option[value="${selectedSizeId}"]`).dataset.name;
        const newRow = document.createElement('tr');
        newRow.innerHTML = `
            <td class="p-2 text-left">${orderCounter++}</td>
            <td class="p-2 text-left hidden">${menuID}</td>
            <td class="p-2 text-left">${itemName} ${selectedAddonName} ${selectedSizeName}</td>
            ...
            <td class="p-2 text-left hidden">${selectedAddonName}</td>
            <td class="p-2 text-left hidden">${selectedSizeId}</td>
