...
@section('content')
<div class="w-full flex flex-col items-center py-4 pt-0">
    <!-- Action Buttons -->
    <div id="navMenu" class="flex flex-wrap justify-center gap-2 hidden md:flex mb-4">
 

    @if(isset($order[0]) && $tableStatus == 'ORDERED')
        <button onclick="openDeleteOrderModal({{ $firstOrder->pos_order_id }})"
                class="bg-red-600 text-white rounded-lg px-4 py-2 text-sm font-semibold hover:bg-red-700 transition">
            Delete Order
        </button>
        <a href="#"
        class="bg-blue-600 text-white rounded-lg px-4 py-2 text-sm font-semibold hover:bg-blue-700 transition">
            Export to Excel
        </a>
    @else
            <a href="javascript:void(0);" 
           onclick="showCreateMenuPopup()" 
           class="bg-bsicolor text-white rounded-lg px-4 py-2 text-sm font-semibold hover:bg-opacity-90 transition">
            Create Menu
        </a>  
    @endif
    ...
    <!-- Divider Line -->
    <div id="menuLine" class="h-1 w-3/4 bg-gray-400 rounded-sm"></div>
</div>
                ...
                <tbody>
                    @if ($tableStatus == 'ORDERED')
                        ...
                        <tr>
                            ...
                            <td class="p-2 text-left">  {{ optional($ordered->Menu)->Menu_name_eng }} {{ optional($ordered->Addons)->Addons_name }} {{ optional($ordered->Size)->Size_name }}
                            </td>
