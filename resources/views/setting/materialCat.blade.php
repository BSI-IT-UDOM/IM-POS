...
<div class="flex flex-col">
  <div class="bg-background flex flex-col items-center flex-grow px-4 md:px-0 mt-2">
    <div class="flex flex-col md:flex-row justify-between items-center w-full md:w-4/5">
      <a href="#" id="createMaterialCate" class="bg-green-500 text-white font-bold py-1 px-4 md:py-2 md:px-6 rounded flex items-center text-sm md:text-base">CREATE</a>
    ...
    <div class="w-full md:w-4/5 border-2 border-bsicolor p-2 font-times">
      <div class="overflow-x-auto">
        <h4 class="text-center font-bold pb-4 text-lg"><u>MATERIAL CATEGORY INFORMATION</u></h4>
        <table class="min-w-full bg-white border-collapse">
          ...
          <tbody>
            ...
            <tr class="{{ $loop->index % 2 === 0 ? 'bg-zinc-200' : 'bg-zinc-300' }} text-base {{ $loop->first ? 'border-t-4' : '' }} text-center border-white">
              ...
              <td class="text-center py-3 px-4 border border-white">{{ $category->Material_Cate_Khname }}</td>
              <td class="text-center py-3 px-4 border border-white">{{ $category->Material_Cate_Engname }}</td>
              <td class="text-center py-3 px-4 border border-white">{{ $category->IMG_id ?? 'null' }}</td>
              <td class="py-3 border border-white">
                <button class="relative bg-blue-500 hover:bg-blue-600 active:bg-blue-700 text-white py-2 px-4 rounded-md focus:outline-none transition duration-150 ease-in-out group "  onclick="EditMaterialCatePopup({{ $category->Material_Cate_id  }}, '{{ $category->Material_Cate_Khname ?? 'null' }}','{{ $category->Material_Cate_Engname ?? 'null'}}')">
                ...
                onclick="confirmDeleteCat({{ $category->Material_Cate_id }})">
                ...
                <button class="relative bg-blue-500 hover:bg-blue-600 active:bg-blue-700 text-white py-2 px-4 rounded-md focus:outline-none transition duration-150 ease-in-out group"
                  onclick="toggleActive(this, {{ $category->Material_Cate_id }})"
  ...
  @include('popups.edit-material-cat-popup')
</div>
  <script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>
  <meta name="csrf-token" content="{{ csrf_token() }}">
<script>
    document.getElementById('createMaterialCate').addEventListener('click', function(event) {
        event.preventDefault();
        document.getElementById('createMaterialCatePopup').classList.remove('hidden');
    });


    function EditMaterialCatePopup(Material_Cate_id, Material_Cate_Khname, Material_Cate_Engname) {
    document.getElementById('editMaterial_Cate_id').value = Material_Cate_id;
    document.getElementById('editMaterial_Cate_Khname').value = Material_Cate_Khname;
    document.getElementById('editMaterial_Cate_Engname').value = Material_Cate_Engname;
    document.getElementById('editMaterialCateForm').action = `/material_category/${Material_Cate_id}`;
...
function toggleActive(button, materialId) {
    ...
    fetch(`/material_category/${materialId}/toggle-status`, {
        method: 'POST',
        headers: {
...
function confirmDeleteCat(Material_Cate_id) {
    Swal.fire({
      title: "Are you sure?",
      ...
      if (result.isConfirmed) {
        window.location.href = `/material_category/destroy/${Material_Cate_id}`;
      }
    });
}
