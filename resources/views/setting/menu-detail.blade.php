    ...
    <div class="container mx-auto p-6">
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 gap-4 mt-2">
            ...
            <div class="bg-white p-2 rounded-lg shadow-md flex flex-col">
                @if(Str::startsWith($value->first()->Menu->Menu_name_eng, 'HOT'))
                <img src="images/hot_drink.png" alt="menu Image" 
                class="w-full h-40 object-contain rounded-lg shadow-md mb-2 transform hover:scale-105 transition duration-300">
                @elseif (Str::startsWith($value->first()->Menu->Menu_name_eng, 'ICED'))
                <img src="images/iced_drink.png" alt="menu Image" 
                class="w-full h-40 object-contain rounded-lg shadow-md mb-2 transform hover:scale-105 transition duration-300">
                @else
                <img src="images/frapped_drink.png" alt="menu Image" 
                class="w-full h-40 object-contain rounded-lg shadow-md mb-2 transform hover:scale-105 transition duration-300">
                @endif
                    ...
                    <div class="mb-2 flex-row">
                        ...
                        <div class="relative group">
                            <button class="bg-blue-500 text-white px-3 py-1 rounded cursor-pointer transition duration-300" 
                                    onclick="openEditMenuDetailPopup({{ $data->pos_menu_detail_id }}, {{ $data->Menu_id ?? 'null' }}, {{ $data->Size_id ?? 'null' }},{{ $data->price ?? 'null'}},{{ $data->Currency_id ?? 'null'}})">
                            ...
                            <button class="bg-red-500 text-white py-1 px-3 rounded cursor-pointer transition duration-300" 
                                    onclick="confirmDeleteDetail({{ $data->pos_menu_detail_id }})">
...
function confirmDeleteDetail(posMenuDetailId) {
    Swal.fire({
      title: "Are you sure?",
      ...
      if (result.isConfirmed) {
        window.location.href = `/menu_detail/destroy/${posMenuDetailId}`;
      }
    });
  }
</script>
