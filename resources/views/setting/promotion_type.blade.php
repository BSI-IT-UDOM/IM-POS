...
<div class="flex flex-col">
  <div class="bg-background flex flex-col items-center flex-grow px-4 md:px-0 mt-2">
    <div class="flex flex-col md:flex-row justify-between items-center w-full md:w-4/5">
      <a href="#" id="createPromotionType" class="bg-green-500 text-white font-bold py-1 px-4 md:py-2 md:px-6 rounded flex items-center text-sm md:text-base">CREATE</a>
    ...
    <div class="w-full md:w-4/5 border-2 border-bsicolor p-2 font-times">
      <div class="overflow-x-auto w-full">
        <h4 class="text-center font-bold pb-4 text-lg"><u>PROMOTION TYPE INFORMATION</u></h4>
        <table class="min-w-full bg-white shadow-md rounded-lg">
          ...
          <tbody id="inventoryTableBody">
            @foreach($getPromotionType as $promotiontype)
            <tr class="{{ $loop->index % 2 === 0 ? 'bg-zinc-200' : 'bg-zinc-300' }} text-base {{ $loop->first ? 'border-t-4' : '' }} text-center border-white">
              ...
              <td class="text-center py-2 px-3 md:py-3 md:px-6 border-b">{{ $promotiontype->promotion_type_name ?? 'null' }}</td>
              <td class="text-center py-2 px-3 md:py-3 md:px-6 border-b">{{ $promotiontype->promotion_type_description ?? 'null' }}</td>
              <td class="py-3 border border-white">
                <button class="relative bg-blue-500 hover:bg-blue-600 active:bg-blue-700 text-white py-2 px-4 rounded-md focus:outline-none transition duration-150 ease-in-out group "  onclick="openPromotionTypePopup({{ $promotiontype->pos_promotion_type_id }}, '{{ $promotiontype->promotion_type_name ?? 'null' }}','{{ $promotiontype->promotion_type_description ?? 'null'}}')">
                ...
                onclick="confirmDelete({{ $promotiontype->pos_promotion_type_id }})">
                ...
                <button class="relative bg-blue-500 hover:bg-blue-600 active:bg-blue-700 text-white py-2 px-4 rounded-md focus:outline-none transition duration-150 ease-in-out group"
                  onclick="toggleActive(this, {{ $promotiontype->pos_promotion_type_id }})"
                  onmouseover="setHover(this, true)"
                  onmouseout="setHover(this, false)"
                  style="background-color: {{ $promotiontype->status === 'Active' ? '#008000' : '#f00' }}; color: white;">
                  <i class="fas {{ $promotiontype->status === 'Active' ? 'fa-toggle-on' : 'fa-toggle-off' }}"></i>
                  <span class="absolute left-1/2 transform -translate-x-1/2 bottom-full mb-2 text-xs text-white bg-gray-600 px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity">
                      {{ $promotiontype->status === 'Active' ? 'Active' : 'Inactive' }}
                  </span>
              </button>
              </td>
            </tr>
...
<script>
    document.getElementById('createPromotionType').addEventListener('click', function(event) {
        event.preventDefault();
        document.getElementById('createPromotionTypePopup').classList.remove('hidden');
    });


    function openPromotionTypePopup(pos_promotion_type_id, promotion_type_name, promotion_type_description) {
    document.getElementById('editpos_promotion_type_id').value = pos_promotion_type_id;    
    document.getElementById('editpromotion_type_name').value = promotion_type_name;
    document.getElementById('editpromotion_type_description').value = promotion_type_description;
    document.getElementById('editPromotionTypeForm').action = `/promotion_type/${pos_promotion_type_id}`;
    document.getElementById('editPromotionTypePopup').classList.remove('hidden');
}

function toggleActive(button, PromotionId) {
    const icon = button.querySelector('i');
    const currentStatus = icon.classList.contains('fa-toggle-on') ? 'Active' : 'Inactive';
    const newStatus = currentStatus === 'Active' ? 'Inactive' : 'Active';

    fetch(`/promotion_type/${PromotionId}/toggle-status`, {
        method: 'POST',
        headers: {
...
function confirmDelete(PromotionId) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            window.location.href = `/promotion_type/destroy/${PromotionId}`;
        }
    });
}
