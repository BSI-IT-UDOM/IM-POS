          ...
          <tbody id="inventoryTableBody">
            ...
            <tr class="{{ $loop->index % 2 === 0 ? 'bg-zinc-200' : 'bg-zinc-300' }} text-base {{ $loop->first ? 'border-t-4' : '' }} text-center border-white">
              ...
              <td class="py-3 border border-white">
                ...
                onclick="confirmDelete({{ $discountdetail->pos_discount_detail_id }})">
...
function confirmDelete(pos_discount_detail_id) {
    Swal.fire({
        title: "Are you sure?",
        ...
        if (result.isConfirmed) {
            window.location.href = `/discount_detail/destroy/${pos_discount_detail_id}`;
        }
    });
}
