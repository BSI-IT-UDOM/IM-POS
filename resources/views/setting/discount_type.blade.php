...
<div class="flex flex-col">
  <div class="bg-background flex flex-col items-center flex-grow px-4 md:px-0 mt-2">
    <div class="flex flex-col md:flex-row justify-between items-center w-full md:w-4/5">
      <a href="#" id="createDiscountType" class="bg-green-500 text-white font-bold py-1 px-4 md:py-2 md:px-6 rounded flex items-center text-sm md:text-base">CREATE</a>
    ...
    <div class="w-full md:w-4/5 border-2 border-bsicolor p-2 font-times">
      <div class="overflow-x-auto w-full">
        <h4 class="text-center font-bold pb-4 text-lg"><u>DISCOUNT TYPE INFORMATION</u></h4>
        <table class="min-w-full bg-white shadow-md rounded-lg">
          ...
          <tbody id="inventoryTableBody">
            ...
            <tr class="{{ $loop->index % 2 === 0 ? 'bg-zinc-200' : 'bg-zinc-300' }} text-base {{ $loop->first ? 'border-t-4' : '' }} text-center border-white">
              ...
              <td class="text-center py-2 px-3 md:py-3 md:px-6 border-b">{{ $discount->discount_type_name ?? 'null' }}</td>
              <td class="text-center py-2 px-3 md:py-3 md:px-6 border-b">{{ $discount->discount_type_description ?? 'null' }}</td>
              <td class="py-3 border border-white">
                <button class="relative bg-blue-500 hover:bg-blue-600 active:bg-blue-700 text-white py-2 px-4 rounded-md focus:outline-none transition duration-150 ease-in-out group "  onclick="openDiscountTypePopup({{ $discount->pos_discount_type_id }}, '{{ $discount->discount_type_name ?? 'null' }}','{{ $discount->discount_type_description ?? 'null'}}')">
                ...
                onclick="confirmDelete({{ $discount->pos_discount_type_id }})">
                ...
                <button class="relative bg-blue-500 hover:bg-blue-600 active:bg-blue-700 text-white py-2 px-4 rounded-md focus:outline-none transition duration-150 ease-in-out group"
                  onclick="toggleActive(this, {{ $discount->pos_discount_type_id }})"
                  onmouseover="setHover(this, true)"
                  onmouseout="setHover(this, false)"
                  style="background-color: {{ $discount->status === 'Active' ? '#008000' : '#f00' }}; color: white;">
                  <i class="fas {{ $discount->status === 'Active' ? 'fa-toggle-on' : 'fa-toggle-off' }}"></i>
  ...
  @include('popups.create-discount-type-popup')
  @include('popups.edit-discount-type-popup')
</div>
<script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>
<script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>
<meta name="csrf-token" content="{{ csrf_token() }}">
<script>
    document.getElementById('createDiscountType').addEventListener('click', function(event) {
        event.preventDefault();
        document.getElementById('createDiscountTypePopup').classList.remove('hidden');
    });


    function openDiscountTypePopup(pos_discount_type_id, discount_type_name, discount_type_description) {
    document.getElementById('editpos_discount_type_id').value = pos_discount_type_id;    
    document.getElementById('editdiscount_type_name').value = discount_type_name;
    document.getElementById('editdiscount_type_description').value = discount_type_description;
    document.getElementById('editDiscountTypeForm').action = `/discount_type/${pos_discount_type_id}`;
    document.getElementById('editDiscountTypePopup').classList.remove('hidden');
}
...
function toggleActive(button, discountId) {
    ...
    fetch(`/discount_type/${discountId}/toggle-status`, {
        method: 'POST',
        headers: {
...
function confirmDelete(discountId) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            window.location.href = `/discount_type/destroy/${discountId}`;
        }
    });
}
</script>
