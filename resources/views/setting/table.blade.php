          ...
          <tbody id="inventoryTableBody">
            ...
            <tr class="{{ $loop->index % 2 === 0 ? 'bg-zinc-200' : 'bg-zinc-300' }} text-base {{ $loop->first ? 'border-t-4' : '' }} text-center border-white">
              ...
              <td class="py-3 border border-white">
                ...
                <button class="relative bg-red-500 hover:bg-red-600 active:bg-red-700 text-white py-2 px-4 rounded-md focus:outline-none transition duration-150 ease-in-out group" onclick="confirmDelete({{ $table->pos_table_id }})">
...
function confirmDelete(tableId) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        window.location.href = `/table_setting/destroy/${tableId}`;
      }
    });
  }
</script>
